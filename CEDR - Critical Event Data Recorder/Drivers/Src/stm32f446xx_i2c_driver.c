...
{
	//add peri-clkcontrol here
	I2C_PeriClockControl(pI2CHandle->pI2Cx, ENABLE);

	volatile uint32_t idr = *(volatile uint32_t*)0x40020410UL;  /* GPIOB->IDR */
	volatile uint8_t idr_scl = (idr >> 10) & 1;   /* PB10 = SCL */
	volatile uint8_t idr_sda = (idr >>  3) & 1;   /* PB3  = SDA */
	...
	*/
	...
	//6. Enable ACK — must be a read-modify-write, and ONLY after PE=1.
	//   (ACK bit is only effective once PE is set.)
